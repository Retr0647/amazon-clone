...
describe('test suite: Appliance', () => {
    ...
    it('has the correct properties', () => {
        expect(applience.id).toEqual('54e0eccd-8f36-462b-b68a-8182611d9add');
        expect(applience.image).toEqual('images/products/black-2-slot-toaster.jpg');
        expect(applience.name).toEqual('2 Slot Toaster - Black');
        expect(applience.rating).toEqual({
            stars: 5,
            count: 2197
        })
        expect(applience.priceCents).toEqual(1899);
        expect(applience.instructionsLink).toEqual('images/appliance-instructions.png');
        expect(applience.warrantyLink).toEqual('images/appliance-warranty.png');
    });

    it('gets the stars url', () => {
        expect(applience.getStarsUrl()).toEqual('images/ratings/rating-50.png');
    });

    it('gets the price', () => {
        expect(applience.getPrice()).toEqual('$18.99');
    });

    it('displays a warranty and instructions links in extraInfoHTML', () => {
        expect(applience.extraInfoHTML()).toContain(`
